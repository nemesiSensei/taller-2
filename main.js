...
/*for (var i = 0; i < datosObject.simpsons.length; i++) 
    {
       if (datosObject.simpsons[i].hair="blond")
       {
            console.log(datosObject.simpsons[i]);
...
{         
    let cabello = document.getElementById('input2').value;    
    ...
    for (var i = 0; i < datosObject.simpsons.length; i++) 
    {
        ...
        if (datosObject.simpsons[i].hair.search(cabello) > -1)
        {            
        var row = tabla.insertRow();
        var cell = row.insertCell(0);
        var cell_1 = row.insertCell(1);
